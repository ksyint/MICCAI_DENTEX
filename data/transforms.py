...
Includes weak and strong augmentations
"""
...
class BottomHalfMask:
    """
    Optionally mask the top half of the image with random probability
    This encourages the model to learn from bottom half but not depend on it entirely
    """
...
def get_transforms(args):
    """
    Get weak, strong, and test transforms
    ...
    Args:
        args: arguments containing:
            ...
            - weak_aug: whether to use weak augmentation
            - strong_aug: whether to use strong augmentation
    
    Returns:
        weak_transform: weak augmentation transform
        strong_transform: strong augmentation transform
        test_transform: test transform (no augmentation)
    """
    img_size = getattr(args, 'img_size', 224)
    mean = getattr(args, 'mean', [0.485, 0.456, 0.406])
    std = getattr(args, 'std', [0.229, 0.224, 0.225])
    
    # Weak augmentation (basic augmentation)
    ...
    strong_transform = transforms.Compose([
        ...
        transforms.ToTensor(),
        # Optionally mask top half to prevent over-dependence
        # BottomHalfMask(mask_prob=0.1),
        transforms.Normalize(mean=mean, std=std),
    ])
...
def get_train_transform(img_size=224, mean=[0.485, 0.456, 0.406], 
    ...
    """Get standard training transform"""
